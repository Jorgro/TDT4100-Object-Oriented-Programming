 ...
 * Units may be derived from each other by a linear formula, e.g. km from m and
 * C (Celcius) from Kelvin.
 * 
 * @author hal
 *
 */
public class Unit {
    ...
    /**
     * Constructor for base units, e.g. meter, gram, kelwin etc. Initialises with
     * the symbol, e.g. "m" for meters, "g" for grams, "K" for kelwin.
     * 
     * @param symbol The symbol of this Unit, must contain only alphabetic
     *               characters.
     * @throws IllegalArgumentException if the symbol contains characters that are
     *                                  not alphabetic
     */
    public Unit(String symbol) throws IllegalArgumentException {
        if (symbol.matches("[A-Za-z]*") && symbol.length() > 0) {
            this.symbol = symbol;
        } else {
    ...
    /**
     * Constructor for derived units, e.g. kilometer, milligram and Celcius, derived
     * from meter, gram and Kelwin respectively. A derived unit includes the factor
     * and offset for the linear formula for computing the base unit from the
     * derived one. base-unit-value = derived-unit-value * factor + offset
     * 
     * @param symbol The symbol for the derived Unit
     * @param base   The base unit, e.g. meter for kilometer, gram for milligram
     * @param factor The factor in the formula, e.g. 1000 for km to m or 0.001 for
     *               mg to g.
     ...
     * @param base   The base unit, e.g. meter for kilometer, gram for milligram
     * @param factor The factor in the formula, e.g. 1000 for km to m or 0.001 for
     *               mg to g. The offset is set to 0.0.
     * @throws IllegalArgumentException if the symbol contains characters that are
     *                                  not alphabetic
     */
     ...
     * derived. If other is derived from this, then this is returned, or if this is
     * derived from other, then other is returned. Otherwise it finds the first base
     * unit that both are derived from.
     * 
     * @param other The other unit.
     * @return The first common unit that is a common base unit for both this and
     *         other.
     */
    public Unit findCommonBaseUnit(Unit other) {
        if (this.equals(other.base)) {
            return this;
        } else if (other.equals(this.base)) {
            return other;
        }
        Unit base1 = this.base;
        Unit base2 = other.base;
    ...
    // The currently supported predefined units, that are considered by the valueOf
    // method
    private static Unit m = new Unit("m"), km = new Unit("km", m, 1000.0), dm = new Unit("dm", m, 0.1),
            cm = new Unit("cm", dm, 0.1), ALL_UNITS[] = { m, km, dm, cm };
